...
def init_threads(threads_dict,num_of_analyser_workers,run_queue_flag,
    ...
    # global error_queue
    error_logger = Thread(target=Worker.error_logger_worker, args=([error_queue,run_queue_flag]))
    error_logger.start()
    threads_dict['error_logger'] = error_logger

    threads_dict['analyser_workers'] = []
    for _ in range(num_of_analyser_workers):
        p = Thread(target=Worker.analyser_worker, args=([tasks_queue,error_queue,updates_queue,to_do_queue,done_queue,run_queue_flag]))
        p.start()
        threads_dict['analyser_workers'].append(p)
    
    task_logger = Thread(target=Worker.task_logger_worker, args=([tasks_queue,error_queue,updates_queue,to_do_queue,run_queue_flag]))
    task_logger.start()
    threads_dict['task_logger'] = task_logger

    results_logger_worker = Thread(target=Worker.results_logger_worker, args=([done_queue,updates_queue,run_queue_flag]))
...
class Worker:
    ...
    @staticmethod
    def results_logger_worker(done_queue,updates_queue,run_queue_flag):
        # global done_queue
        # global run_queue_flag
        # global updates_queue
